...
import type { TestResult, TestResultItem } from "../types.js";
...
export function parseForgeOutput(output: string, success: boolean): TestResult {
...
function isHardhatTest(content: string): boolean {
  return (
...
`;
    ...
    for (const contract of contracts) {
      await fs.writeFile(
        path.join(projectDir, "src", contract.name),
        contract.content
      );
    }
    ...
    for (const test of tests) {
      if (isHardhatTest(test.content)) {
        return {
          ...
          failed: 0,
          output:
            "Error: Test file is in Hardhat/JavaScript format. Please regenerate tests in Foundry format.",
          tests: [],
        };
      }
      ...
      await fs.writeFile(
        path.join(projectDir, "test", testName),
        test.content
      );
    }
