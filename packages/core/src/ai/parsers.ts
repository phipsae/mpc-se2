...
export function parseContracts(responseText: string): { name: string; content: string }[] {
...
export function parsePages(responseText: string): { path: string; content: string }[] {
...
export function parseTests(responseText: string): { name: string; content: string }[] {
  ...
  if (tests.length === 0) {
    ...
    for (const match of solBlocks) {
      ...
      if (isHardhatTest(content)) continue;
...
export function isHardhatTest(content: string): boolean {
  return (
