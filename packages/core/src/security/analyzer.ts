...
export function analyzeSecurityPatterns(
  ...
  for (const contract of contracts) {
    ...
    if (
      source.match(/pragma solidity\s*\^\s*0\.[0-7]/) &&
    ...
    if (
      source.includes("function withdraw") &&
...
export function estimateGas(bytecode: string): {
  estimated: string;
  costEth: string;
  costUsd: string;
} {
...
export function checkSize(bytecode: string): {
  bytes: number;
  kb: string;
  withinLimit: boolean;
} {
