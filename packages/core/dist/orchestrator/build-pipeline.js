...
export async function buildDApp(opts) {
    const { prompt, plan, existingCode, maxIterations = MAX_TOTAL_ITERATIONS, onProgress, } = opts;
    const logs = [];
    ...
    try {
        ...
        if (existingCode?.contracts && existingCode.contracts.length > 0) {
            ...
            log(`Generated ${contracts.length} contracts, ${tests.length} tests`);
        }
        ...
        while (!compileResult.success &&
            ...
            log(`Compilation failed (attempt ${compilationAttempts}), fixing errors...`, "fixing_compilation");
        ...
        while (securityWarnings.length > 0 &&
            ...
            if (Date.now() - startTime > BUILD_TIMEOUT_MS)
                throw new Error("Build timeout");
            log(`Found ${securityWarnings.length} security issues (attempt ${securityAttempts}), fixing...`, "fixing_security");
        ...
        let testAttempts = 0;
        let testResult = await runForgeTests(contracts, tests);
        while (!testResult.success && testAttempts < MAX_TEST_ATTEMPTS) {
            ...
            if (Date.now() - startTime > BUILD_TIMEOUT_MS)
                throw new Error("Build timeout");
            log(`Tests failed (attempt ${testAttempts}): ${testResult.failed} failures. Fixing...`, "fixing_tests");
        ...
        log(`Build complete in ${(elapsed / 1000).toFixed(1)}s with ${totalIterations} iterations`, "done");
        return {
