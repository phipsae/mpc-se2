...
export async function createVercelDeployment(accessToken, githubRepoUrl, projectName) {
    try {
        ...
        const existingProjectResponse = await fetch(`https://api.vercel.com/v9/projects/${projectName}`, { headers: { Authorization: `Bearer ${accessToken}` } });
        if (existingProjectResponse.ok) {
            const existingProject = await jsonBody(existingProjectResponse);
            const projectDetailsResponse = await fetch(`https://api.vercel.com/v9/projects/${existingProject.id}`, { headers: { Authorization: `Bearer ${accessToken}` } });
            if (projectDetailsResponse.ok) {
                ...
                if (repoId) {
                    ...
                    if (deployResponse.ok) {
                        ...
                        try {
                            const deploymentUrl = await waitForDeployment(accessToken, deployment.uid || deployment.id, 180000);
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            if (projectResponse.ok) {
                ...
                const deploymentsResponse = await fetch(`https://api.vercel.com/v6/deployments?projectId=${project.id}&limit=1`, { headers: { Authorization: `Bearer ${accessToken}` } });
                ...
                if (latestDeployment) {
                    try {
                        const deploymentUrl = await waitForDeployment(accessToken, latestDeployment.uid || latestDeployment.id, 180000);
...
async function waitForDeployment(accessToken, deploymentId, timeout) {
    const startTime = Date.now();
    while (Date.now() - startTime < timeout) {
        const response = await fetch(`https://api.vercel.com/v13/deployments/${deploymentId}`, { headers: { Authorization: `Bearer ${accessToken}` } });
        if (!response.ok)
...
export function generateProjectName(baseName) {
    return baseName
        .toLowerCase()
        .replace(/[^a-z0-9-]/g, "-")
        .replace(/-+/g, "-")
