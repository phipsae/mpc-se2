...
function isOctokitError(error) {
    return (error instanceof Error &&
        typeof error.status === "number");
}
export async function createRepoAndPushFiles(accessToken, repoName, description, files, retryCount = 0) {
...
export async function updateRepoFiles(accessToken, repoUrl, files, commitMessage = "Update from AI dApp Builder") {
    const octokit = new Octokit({ auth: accessToken });
    try {
...
export function generateRepoName(contractName) {
