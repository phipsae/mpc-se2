...
export function createServer(ctx) {
    ...
    // 2. check_security
    server.tool("check_security", "Run security pattern analysis (reentrancy, unchecked calls, tx.origin, etc.) and gas estimation on Solidity contracts.", {
        contracts: z.array(z.object({ name: z.string(), content: z.string() })),
    ...
    // 3. run_tests
    server.tool("run_tests", "Run Foundry/Forge tests. Creates a temporary Foundry project, installs OpenZeppelin, and runs `forge test -vvv`. Returns per-test pass/fail results.", {
        contracts: z.array(z.object({ name: z.string(), content: z.string() })),
    ...
    // 4. assemble_project
    server.tool("assemble_project", "Build a complete Scaffold-ETH 2 project directory from contracts, tests, and pages. Copies the SE2 template and injects generated code. Returns project path on disk.", {
        ...
        try {
            const result = await assembleProjectTool.handler(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        }
        catch (e) {
            return { content: [{ type: "text", text: JSON.stringify({ error: e.message }) }], isError: true };
    ...
    // 5. start_anvil
    server.tool("start_anvil", "Start a local Anvil testnet. Returns RPC URL and funded test accounts with private keys.", {
    ...
    // 6. stop_anvil
    server.tool("stop_anvil", "Stop a running Anvil testnet instance.", {
        projectId: z.string().describe("Project ID"),
    }, async (args) => {
        try {
    ...
    // 7. deploy_local
    server.tool("deploy_local", "Deploy contracts to any EVM network (local Anvil or remote like Sepolia/Base/Optimism) using forge script. Requires assemble_project first.", {
        projectId: z.string().describe("Project ID"),
    ...
    // 8. push_github
    server.tool("push_github", "Push an assembled project to a new GitHub repository.", {
        projectId: z.string().describe("Project ID"),
